...
class CustomLSTMCell(DropoutRNNCellMixin, Layer):
  ...
  def call(self, inputs, states, training=None):
    ...
    dp_mask = self.get_dropout_mask_for_cell(gcnout, training, count=4)
    rec_dp_mask = self.get_recurrent_dropout_mask_for_cell(
        h_tm1, training, count=4)

    if self.implementation == 1:
      if 0 < self.dropout < 1.:
        inputs_i = gcnout * dp_mask[0]
        inputs_f = gcnout * dp_mask[1]
        inputs_c = gcnout * dp_mask[2]
        inputs_o = gcnout * dp_mask[3]
      else:
        inputs_i = gcnout
        inputs_f = gcnout
        inputs_c = gcnout
